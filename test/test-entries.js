...
function seedJournalEntriesData() {
	console.info('Seeding Journal Entries data');
	const seedData = [];

	for (let i=1; i<=10; i++) {
		seedData.push(generateJournalEntries());
	};
...
describe('Journal entries API resource', function() {
	...
	describe('GET endpoint', function() {
		...
		it('should return entries with right fields', function() {
			//Strategy: Make GET req and ensure they entries have the right fileds
			let resEntry;
			return chai.request(app)
			.get('/api/entries')
			.set('Authorization', `Bearer ${jwt}`)
			.then(function(res) {
				...
				res.body.entries.forEach(function(entry) {
					...
					expect(entry).to.include.keys('id', 'title', 'travelDate', 'coverPhoto',
					'description', 'memories', 'words', 'morePhotos');
				});
	...
	describe('POST endpoint', function() {
		...
		it('should add a new entry', function() {
			...
			.then(function(res) {
				...
				expect(res.body).to.include.keys('id', 'title', 'travelDate', 'coverPhoto',
					'description', 'memories', 'words', 'morePhotos');
	...
	describe('PUT endpoint', function() {
		...
		// 2. Maake a PUT req to update that entry
